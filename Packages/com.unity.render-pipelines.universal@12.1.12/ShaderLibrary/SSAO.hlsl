...
#include "Assets/Res/Shader/Common/DeclareOutlineDepthTexture.hlsl"
...
half3 PickSamplePoint(float2 uv, int sampleIndex)
{
    ...
    const half gn = half(GetRandomUVForSSAO(half(0.0), sampleIndex));
...
{
    float rawDepth = SampleSceneOutlineDepth(uv.xy);
...
half4 SSAO(Varyings input) : SV_Target
{
    ...
    for (int s = 0; s < SAMPLE_COUNT; s++)
    {
        ...
        // half3 spos_s2 = mul(camTransform, vpos_s2);
        // #if defined(_ORTHOGRAPHIC)
        //     float2 uv_s2_01 = clamp((spos_s2.xy + float(1.0)) * float(0.5), float(0.0), float(1.0));
        // #else
        //     float zdist2 = -dot(UNITY_MATRIX_V[2].xyz, vpos_s2);
        //     float2 uv_s2_01 = clamp((spos_s2.xy * rcp(zdist2) + float(1.0)) * float(0.5), float(0.0), float(1.0));
        // #endif
        // float depth_s2 = SampleAndGetLinearEyeDepth(uv_s1_01);
        ...
        half a3 = smoothstep(RADIUS * 3.0, RADIUS * 0.5, a2);
        ao += a1 * rcp(a2) * a3;
    }
