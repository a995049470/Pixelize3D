...
using Lost.Runtime.Footstone.Diagnostics;

namespace Lost.Runtime.Footstone.Reflection
{
    ...
    public static class AssemblyRegistry
    {
        ...
        public static Type FindTypeByName(string fullName, string assemblyName = "Assembly-CSharp")
        {
            Type type = null;
            if(AssemblyNameToAssembly.TryGetValue(assemblyName, out var assembly))
            {
        ...
        public static void Register(Assembly assembly, IEnumerable<string> categories)
        {
            ...
            lock (Lock)
            {
                ...
                foreach (var category in categories)
                {
                    ...
                    {
                        LostDebug.LogError($"Invalid empty category for assembly [{assembly}]");
                        continue;
                    }
