...
namespace Lost.Runtime.Footstone.Core
{
    ...
    public abstract class EntityManager 
    {
        ...
        //entity的字典， 可以通过id对Entity进行索引
        private readonly Dictionary<ulong, Entity> entityDic;
        ...
        protected EntityManager(IServiceRegistry service)
        {
            Services = service;
            entities = new HashSet<Entity>();
            entityDic = new Dictionary<ulong, Entity>();
        ...
        public virtual void Update(GameTime gameTime)
        {
            ...
            var originCount = processors.Count;
            for (int i = 0; i < originCount; i++)
            {
                var processor = processors[i];
                if (processor.Enabled)
                {
                    if(processor.WaitRunTheFirstFrame)
                    {
                        processor.WaitRunTheFirstFrame = false;
                        processor.BeforeProcessorRunTheFirstFrame();
                    }
                    processor.Update(gameTime);
                }

                //可能在update过程中在i位置之前插入processor
                var currentCount = processors.Count;
                if(currentCount != originCount)
                {
                    for (int j = 1; j < currentCount - originCount + 1; j++)
                    {
                        if(processors[j + i] == processor)
                        {
                            i += j;
                            break;
                        }
        ...
        /// <summary>
        /// 强制获取Processor, 获取失败会直接创建 如果有循环依赖估计会卡死
        /// </summary>
        /// <typeparam name="TProcessor"></typeparam>
        /// <returns></returns>
        public TProcessor ForceGetProcessor<TProcessor>() where TProcessor : EntityProcessor
        {
            var processor = Processors.Get<TProcessor>();
            if(processor == null)
            {
                var processorType = typeof(TProcessor);
                processor = (TProcessor)Activator.CreateInstance(processorType);
        ...
        {
            InternalRemoveEntity(entity, false);
        }
        ...
        /// <summary>
        /// Adds the entity.
        /// 只在两种情况下出发 Enity.OnEnable 或 Enity.Scene 赋值时
        ...
        public void Add(Entity entity)
        {
            ...
            //     throw new System.ArgumentException("Entity shouldn't have a parent.", nameof(entity));
        ...
        internal void InternalAddEntity(Entity entity)
        {
            ...
            {
                var src = entityDic[entity.Id];
                UnityEngine.Debug.Log($"{entity.name},  {src.name}", entity.gameObject);
            }
        ...
        internal void OnProcessorAdded(EntityProcessor processor)
        {
        ...
        internal void OnProcessorRemoved(EntityProcessor processor)
        {
        ...
        public void ChangeEntityId(Entity entity, ulong newId)
        {
            entityDic.Remove(entity.Id);
            entity.ResetUniqueId(newId);
        ...
        public bool IsVaildEntityUID(ulong id)
        {
