...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	{
		float perspectiveDpeth = 1.0 / (zBufferParam.z * depth + zBufferParam.w);
		
	#if UNITY_REVERSED_Z
		float orthoDpeth = lerp(_ProjectionParams.y, _ProjectionParams.z, 1 - depth);
	#else
		float orthoDpeth = lerp(_ProjectionParams.y, _ProjectionParams.z, depth);
	#endif
		return lerp(perspectiveDpeth, orthoDpeth, unity_OrthoParams.w);
	}
